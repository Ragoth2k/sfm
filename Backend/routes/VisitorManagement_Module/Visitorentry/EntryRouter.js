...
//     try {
//       const [visitor] = await db.query('SELECT * FROM visitor_management WHERE visitor_code = ?', [visitor_code]);
...
//     // const check_in = new Date().toISOString().slice(0, 16);
...
//         return res.status(404).json({ Status: false, Error: 'Visitor not found' });
//       }
  
//       res.json({ Status: true, Message: 'Check out time recorded successfully' });
//     } catch (err) {
//       console.error("Error updating check out time:", err);
//       res.status(500).json({ Status: false, Error: 'Error recording check out time' });
//     }
//   });
...
router.post('/get_visitor_by_code', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const [existingVisitor] = await db.query('SELECT * FROM visitor_entry WHERE visitor_code = ?', [visitor_code]);

    if (existingVisitor.length) {
      return res.status(409).json({ message: 'Visitor has already been checked in.' });
    }
